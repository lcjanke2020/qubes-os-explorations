...
- **A dom0 update can break — or fix — passthrough at the host level; back up dom0 first, and stay current.** Separate from the guest-side offset-4 fall-off (§6): a dom0 update once regressed passthrough so that starting the GPU qube **black-screened and kernel-panicked dom0 *itself*** (~1–2 s after assignment; dom0 was fine while the qube stayed stopped) — no BIOS or driver change fixed it, and a **later dom0 update did**, root cause never pinned down. Two takeaways: **(1) always take a Qubes backup (`qvm-backup`, dom0 included) before every dom0 update** — we skipped it and paid for it, since the resulting crash loop left stale `vm-*-volatile` LVs / a wedged thin-pool to untangle (see the previous bullet); and **(2) keep dom0 on the latest Qubes update**, since a dom0 update is what both broke and later fixed this.
